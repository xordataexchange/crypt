...
func getPlainCmd(flagset *flag.FlagSet) {
	flagset.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: %s getplain [args...] key\n", os.Args[0])
		flagset.PrintDefaults()
	}
	flagset.Parse(os.Args[2:])
	key := flagset.Arg(0)
...
func setPlainCmd(flagset *flag.FlagSet) {
	flagset.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: %s setplain [args...] key file\n", os.Args[0])
		flagset.PrintDefaults()
	}
	flagset.Parse(os.Args[2:])
	key := flagset.Arg(0)
	...
	if err := backendStore.Set(key, value); err != nil {
		log.Fatal(err)
	}
}
